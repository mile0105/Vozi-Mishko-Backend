...
INSERT INTO CITY(ENGLISH_NAME, MACEDONIAN_NAME, ALBANIAN_NAME) VALUES ('SKOPJE', 'СКОПЈЕ', ''),
                        ('BITOLA', 'БИТОЛА',''),
                        ('KUMANOVO', 'КУМАНОВО',''),
                        ('PRILEP', 'ПРИЛЕП',''),
                        ('TETOVO', 'ТЕТОВО',''),
...
ALTER TABLE TRIPS ADD CONSTRAINT START_CITY_ID_FK FOREIGN KEY(START_CITY_ID) REFERENCES CITY(ID);
ALTER TABLE TRIPS ADD CONSTRAINT END_CITY_ID_FK FOREIGN KEY(END_CITY_ID) REFERENCES CITY(ID);
